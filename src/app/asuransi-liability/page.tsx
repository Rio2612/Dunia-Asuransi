...
import { Shield, ArrowLeft, Scale, Phone, MessageCircle, CheckCircle, Users, Package, Briefcase, Leaf, ChevronDown } from 'lucide-react'
...
// FAQ Data untuk Schema dan Tampilan
const faqData = [
  {
    question: 'Apa itu Asuransi Liability?',
    answer: 'Asuransi Liability adalah jenis asuransi yang memberikan perlindungan terhadap risiko tuntutan hukum dari pihak ketiga akibat aktivitas bisnis. Asuransi ini menanggung biaya ganti rugi, biaya pembelaan hukum, dan pengeluaran lain yang terkait dengan klaim tanggung jawab hukum. Produk ini sangat penting bagi perusahaan yang berinteraksi dengan publik, konsumen, atau klien.'
  },
  {
    question: 'Apa saja jenis-jenis Asuransi Liability?',
    answer: 'Jenis-jenis Asuransi Liability meliputi: Public Liability (tuntutan akibat cedera atau kerusakan properti di area operasional), Product Liability (tuntutan akibat produk cacat), Professional Indemnity (tuntutan akibat kesalahan profesional), Environmental Liability (tuntutan akibat pencemaran lingkungan), dan Directors & Officers Liability (tuntutan terhadap direksi dan komisaris).'
  },
  {
    question: 'Siapa yang membutuhkan Asuransi Liability?',
    answer: 'Asuransi Liability dibutuhkan oleh berbagai pihak termasuk: pemilik gedung, hotel, mall, rumah sakit (Public Liability), produsen dan distributor (Product Liability), konsultan, akuntan, arsitek, pengacara (Professional Indemnity), industri kimia dan pengelola limbah B3 (Environmental Liability), serta perusahaan dengan dewan direksi (D&O Liability).'
  },
  {
  ...
  },
  {
    ...
    answer: 'Ya, Asuransi Liability umumnya mencakup biaya pembelaan hukum termasuk biaya pengacara, biaya pengadilan, dan pengeluaran lain yang terkait dengan proses hukum. Biaya-biaya ini biasanya dibayar langsung oleh perusahaan asuransi atau diganti setelah klaim diselesaikan, tergantung ketentuan polis.'
  },
  {
          ...
          <article className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg p-6 md:p-10">
            ...
            <div className="mt-6 space-y-3">
              {faqData.map((faq, index) => (
                <details key={index} className="group bg-slate-50 rounded-lg">
                  <summary className="flex items-center justify-between cursor-pointer p-4 font-medium text-slate-800 hover:bg-slate-100 rounded-lg">
                    {faq.question}
